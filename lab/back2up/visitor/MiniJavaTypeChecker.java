...
public class MiniJavaTypeChecker extends GJNoArguDepthFirst<String> implements GJNoArguVisitor<String>{
    ...
    private boolean cextends(String c, String p) {
        if(lookupclass(c) != "class") return false;
        if(lookupclass(p) != "class") return false;
        if(c == p) return true;
    ...
    private String lub(String class1, String class2){
        ...
        if(lookupclass(class1) != "class") return null;
        if(lookupclass(class2) != "class") return null;
...
/**
    ...
    private boolean compareListsHeritage(ArrayList<String> a, ArrayList<String> b) {
        ...
        if(a.size() != b.size()) return false;
        for(int i = 0; i < a.size(); i++) {
            if((lookupclass(a.get(i)) == "class")&&(lookupclass(b.get(i)) == "class")) {
    ...
    private boolean validdatemethod(String mname, String bclass, String access,String rettype, ArrayList<String> argtypes) {
        ...
        while(pclass != null) {
            ...
            if(scopes.get(scope).contains(mname)){
                ...
                if(lookupclass(typeof.get(fname)) == "class" 
                && lookupclass(rettype) == "class") {
        ...
        for (String string : typeof.keySet()) {
            if(typeof.get(string) != "class") continue;
    ...
    */
    public String visit(Goal n) {
     ...
     public String visit(MethodDeclaration n) {
        ...
        if(typecheck){
            if(lookupclass(retval) == "class" && lookupclass(rettype) == "class") {
 ...
 public String visit(ProtectedType n) {
    ...
    return _ret;
     ...
     public String visit(AssignmentStatement n) {
        ...
        if(typecheck){
            if(lookupclass(type) == "class" && lookupclass(varname) == "class") {
     ...
     public String visit(MessageSend n) {
        ...
        if(typecheck) {
            ...
            if(lookupclass(classname) != "class") error();
        }
        n.f1.accept(this);
